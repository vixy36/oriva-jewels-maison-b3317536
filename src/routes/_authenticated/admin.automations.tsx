...
import { Plus, Trash2, Pencil, Zap, AlertCircle } from "lucide-react";

export const Route = createFileRoute("/_authenticated/admin/automations")({
  component: AutomationsAdmin,
});
...
function AutomationsAdmin() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from("marketing_automations").select("*").order("trigger_type").order("trigger_status");
    ...
    <div>
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <h1 className="mt-2 font-serif text-3xl">Email Automations</h1>
      ...
      <div className="mt-6 border border-amber-500/30 bg-amber-500/5 rounded p-4 flex items-start gap-3">
        ...
        <div className="text-sm">
          ...
          <p className="text-muted-foreground mt-1">
            Automations save and run, but actual email delivery only starts after you complete the email domain setup.
            Ask the maison to configure emails from the project settings.
          </p>
        </div>
      </div>
