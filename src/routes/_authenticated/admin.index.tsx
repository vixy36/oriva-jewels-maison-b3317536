...
import { Package, Inbox, Search, FileEdit } from "lucide-react";
...
function Dashboard() {
  const [stats, setStats] = useState({ products: 0, enquiries: 0, unread: 0, seo: 0, content: 0 });

  useEffect(() => {
    (async () => {
      const [p, e, u, s, c] = await Promise.all([
  ...
  const cards = [
    ...
    { to: "/admin/content", label: "Content Blocks", value: stats.content, icon: FileEdit },
  ];
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-muted-foreground">Manage products, SEO, enquiries and site content.</p>
