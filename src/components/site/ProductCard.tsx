  ...
  return (
    ...
    >
      <div className="relative overflow-hidden bg-charcoal aspect-[4/5]">
        ...
        <span className="absolute top-4 left-4 text-[9px] tracking-[0.35em] uppercase text-ivory/70 bg-obsidian/60 backdrop-blur-sm px-2.5 py-1 border border-white/10">
          {product.diamondTypes.includes("Lab Grown") && product.diamondTypes.includes("Natural")
            ? "Natural · Lab"
            : product.diamondTypes[0]}
        </span>

        <span className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-ivory text-obsidian px-5 py-2.5 text-[10px] tracking-[0.32em] uppercase opacity-0 group-hover:opacity-100 translate-y-3 group-hover:translate-y-0 transition duration-500 whitespace-nowrap">
      ...
      <div className="mt-6 flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-[9.5px] tracking-[0.4em] uppercase text-gold">
            {product.collection}
          </p>
        ...
        <span className="mt-2 shrink-0 text-[10px] tracking-[0.32em] uppercase text-ivory/40">
          Enquire
        </span>
      </div>
    </Link>
